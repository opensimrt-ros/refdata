        ...
        for session in glob.glob(pre_path+"/SESSION*"):
            ...
            for action in ["walking","sts","squat"]:
                ...
                if not os.path.isfile(pruned_files):
                    ...
                    real_output_dir = f"./output_pruned/{sub}/{ses}/{action}"
                    if not os.path.isdir(real_output_dir):
                        os.makedirs(real_output_dir)

                try:
                    generate_graph_grf.run(trial_data,pruned_files,real_output_dir)
                    generate_graph_ik.run(trial_data,pruned_files, real_output_dir)
                    generate_graph_id.run(trial_data,pruned_files, real_output_dir)
                    generate_graph_so.run(trial_data,pruned_files, real_output_dir)
                except:
