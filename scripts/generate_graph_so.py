...
def run(trial_data_yaml,trial_files_yaml,out_dir):
    ...
    if False:
        xy_knees_so = refdata.generate_somejoint_or_muscle_curves(action_trials,[], curve_prefix="ankle_angle")
    ...
    xy_time_clippings_left = build_clipings(so_files,left_times)
    ...
    for refref in ref.items():
        for conv_names in cc:
            for std in [True,False]:
                GRAPH_NAME = f'so_{refref[0]}_ref'

                fdp_graph(action_trials,xy_clippings_both_so, GRAPH_NAME, subject_num,out_dir,this_action_name,actions_to_be_shown,this_action_name,conv_names=conv_names[0],grid=conv_names[1],std=std, ref=refref[1][0],subject_identifier=refref[1][1])
